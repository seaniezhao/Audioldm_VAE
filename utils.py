...
def get_mel_from_wav_file(file_path):
    ...
    if mel_fmax not in mel_basis:
        ...
        hann_window[str(y.device)] = torch.hann_window(win_length).to(
            y.device
        )
    ...
    mel = spectral_normalize_torch(
        torch.matmul(
            mel_basis[str(mel_fmax) + "_" + str(y.device)], stft_spec
        )
    )

    mel = mel[0].T
    target_length = int(duration * sampling_rate / hop_length)
    pad_mel = pad_spec(mel, target_length)
